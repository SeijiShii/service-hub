...
// service-info レスポンス契約（[論点-T1] 解決、providers SPEC §1.3 で確定。最小固定 + extra）
// v2 (favicon-projection, 2026-05-28): iconUrl?: string を 1st-class field として追加。
//   v1 受信は完全許容 (iconUrl 未定義として扱う、後方互換)。format check は src/lib/safeUrl.ts。
type ServiceInfoStatus = 'ok' | 'degraded' | 'down';
interface ServiceInfoResponse {
  schemaVersion: number;   // 必須・後方互換判定 (v1=1 / v2=2、receiver は両方許容)
  service: string;         // 必須・slug
  status: ServiceInfoStatus;
  metrics?: Array<{ key: string; value: number; unit: string }>;
  version?: string;
  iconUrl?: string;        // v2: producer の favicon 絶対 URL (https + 公開ホスト + 1024 chars 以内)
...
interface ServiceDescriptor {
  ...
  iconUrl?: string;      // favicon-projection (2026-05-28): producer 自己申告、admin write 不可 (SoT 一貫性)
}

// producer 申告メタ (favicon-projection、ProviderAdapter.collect 戻り値の meta? に格納)
...
// favicon-projection (2026-05-28): meta?: ServiceMeta 追加。service-info adapter のみ producer 申告メタ
// (iconUrl 等) を返し、runner で services テーブルへ永続化集約。ping/vercel/neon は meta 返さず undefined。
interface ProviderAdapter {
  ...
  collect(service: ServiceDescriptor): Promise<{
    metrics: UsageMetric[];
    error?: string;
    meta?: ServiceMeta;  // favicon-projection: 副作用は runner で集約 (spec-review R1)
  }>;
}
```
...
## 8. 未決事項（論点リスト）
### [論点-T1] service-info 契約型の詳細（[論点-003] 連動）— ✅ 解決 (2026-05-26)
- **解決**: providers SPEC §1.3 で契約スキーマ確定（最小固定 + extra）→ `ServiceInfoResponse`/`ServiceInfoStatus` を §1.2 に追加済。
- **残**: 実装時に `src/types/service.ts` へ反映（providers の tdd と同時）。

## 9. 更新履歴
| 日付 | 変更概要 | 実行者 |
|---|---|---|
| 2026-05-26 | 初版作成（/flow:auto P4 dispatch、Phase 2 最初の基盤） | /flow:feature |
| 2026-05-28 | favicon-projection 反映: ServiceInfoResponse v2 (iconUrl?) + ServiceDescriptor.iconUrl? + ServiceMeta 新規 + ProviderAdapter 戻り値 meta?。詳細は `revise_favicon-projection_20260528/` | /flow:tdd (revise) |
