...
| docs/feedback-inbox/ | **★★★必須** 各サービスのユーザーフィードバック/問い合わせを横断閲覧する運営者インボックス（perspectives O67 consumer、[論点-007]）。各サービスの `GET /api/hub/feedback`（O66）を `HUB_SERVICE_INFO_SECRET` で pull → `FeedbackItem` 型で保存 → サービス別フィルタ + トリアージ（`/flow:claim`）導線。Shipyard 問い合わせは標準契約外のため**専用 adapter** で取り込む（shipyard concept §で定義） | `/feedback` 運営者インボックス画面 + feedback pull adapter（detect signal: `/api/hub/feedback` / `FeedbackItem` / `feedbackInbox`） | _shared/types, _shared/db, _shared/auth, collection, registry | 4 | ❌ |
...
### 6.2 feedback/問い合わせ契約のクロスサービス波及（運営者閲覧、[論点-007]、2026-06-18）

各サービスは O40 でユーザーフィードバック/問い合わせを収集するが、**運営者（seiji）が横断して閲覧する仕組みが無かった**（O40 が構想した別 PJ feedback-hub は未構築 = 各サービス DB に溜まるだけの死角）。**ServiceHUB を運営者の閲覧面にする**（seiji [flow] 2026-06-18）。service-info（§6.1）と完全同型の **pull / 共有シークレットモデル**を採用（perspectives O66 producer ↔ O67 consumer ペア）:

1. **producer 契約（O66）**: 各サービスが `GET /api/hub/feedback` を公開（service-info `/api/hub/service-info` とは**別エンドポイント** = 件数・PII・保持期間が指標と別管理）。認証は service-info と同じ**全サービス共通シークレット 1 本** `HUB_SERVICE_INFO_SECRET`。レスポンス = `{ schemaVersion, service, items: [{ id, kind: "feedback"|"bug"|"inquiry", body, rating?, context?, createdAt, status? }], nextCursor?, extra? }`（最小固定 + extra、PII scrub 済み）。
2. **consumer 配線（O67 = 本 HUB の責務、[論点-007]）**: HUB が (a) 各サービスの `/api/hub/feedback` を `HUB_SERVICE_INFO_SECRET` で **pull**（service-info pull と同層 adapter）、(b) `_shared/types` に `FeedbackItem` 型を定義し受信値を保存、(c) **運営者向け（Clerk ゲート内）フィードバック/問い合わせインボックス画面**（`docs/feedback-inbox/`、横断一覧 + サービス別フィルタ + `/flow:claim` トリアージ導線）を提供する。
3. **Shipyard 専用 adapter**: Shipyard（givers.work showcase）の問い合わせはプラットフォーム全体の contact form で per-service feedback 契約に乗らない → **そこだけ専用 adapter** で取り込む（shipyard 側 API 設計 + HUB 側 adapter）。標準 O66 契約とは別経路。shipyard concept に PJ 固有要件として登録済み。
4. **後方互換**: 未実装サービスは空 items / 404 にフォールバック（HUB は producer 順次対応を許容）。
5. **audit 検出**: producer 未実装は O66、HUB の consumer 未配線は **O67（`required_signals=[/api/hub/feedback, FeedbackItem]` + 集約 HUB `_when`）** が audit #4 で検出。本 §6.2 + `docs/feedback-inbox/`（§1.3.1）が PJ 側の audit-hittable 登録（§2.5、CF-20260618-006）。
...
| 2026-06-18 | **ユーザーフィードバック/問い合わせを ServiceHUB で運営者が閲覧する pull 契約を採用**（[論点-007] open）。各サービスは O40 で収集するが横断閲覧の仕組みが無かった（O40 構想の feedback-hub 未構築 = 死角）。ServiceHUB を閲覧面にする。転送モデル = service-info（§6.1）と同型の **pull / 共有シークレット**（seiji 選択、代替の service-info 内包・push feedback-hub は却下）。perspectives O66（producer = `GET /api/hub/feedback`）↔ O67（consumer = HUB の pull adapter + `FeedbackItem` + `docs/feedback-inbox/` 運営者画面）ペアを SoT 化。Shipyard 問い合わせは標準契約外で専用 adapter（shipyard concept）。実装は未着手 = audit #4（O67）が追跡 | seiji [flow]「各種サービスのフィードバック/問い合わせを運営者が ServiceHUB で閲覧できるように。Shipyard はそこだけ専用 API。ServiceHUB 契約の修正。audit 検知されるように」(CF-20260618-006) | §1.3.1, §6.2, §8 論点-007, `_shared/types`, feedback-inbox | [flow] CF-20260618-006 |
...
- **detect signal（audit がこれで未実装を検出、perspectives O67 `required_signals`）**: コードベースに以下が**全て**存在すること（不足 = High consumer 未配線）:
  1. 各サービスの `GET /api/hub/feedback` を `HUB_SERVICE_INFO_SECRET` で pull する adapter（signal: `/api/hub/feedback`）
  2. `_shared/types` に `FeedbackItem` 型（受信値の保存・表示）
  3. 運営者向けインボックス画面（`docs/feedback-inbox/`、Clerk ゲート内、横断一覧 + サービス別フィルタ + `/flow:claim` 導線。signal: `feedbackInbox`）
  4. Shipyard 問い合わせ用の専用 adapter（標準 O66 契約外。shipyard 側 API + HUB 側取り込み）
- **producer 側（各マイクロサービス）**: O66 が各サービスに `GET /api/hub/feedback` 実装を課す（`/flow:revise`）。本論点は consumer（HUB）側のみ。
- **推奨修正アクション**: `/flow:feature docs/feedback-inbox`（新規機能フォルダ）で pull adapter + `FeedbackItem` 型 + 運営者インボックス画面を実装（Class A）。Shipyard adapter は shipyard 側 API 設計（別 repo tracked）と HUB 側取り込みをセットで。
- **判断期限**: 次回 service-hub 改修サイクル（実装で close）
- **担当**: seiji
- **由来**: seiji [flow] 2026-06-18 / CF-20260618-006 / perspectives O66・O67
