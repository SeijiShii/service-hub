...
- 現在フェーズ: **Phase 4 (Release gate ✅ 16th deploy 完了)** — 16 回 deploy 完了済、本番 https://service-hub.givers.work 稼働中 (internal tool)。13th〜16th deploy (2026-06-07/08) = 収益指標表示 (C20260607-001) / 収益推移chart + collect unit-resilience hotfix (C20260607-002) / chart-ux (時間軸統一+期間選択+usd系削除) / chart-colors (線色palette暖寒交互)。直近 (2026-06-18) に summary-projection [論点-011] 実装完了 (未デプロイ、下記)。
- 進行中ターゲット: **summary-projection [論点-011] 実装完了・prod 反映待ち** (commit 8e97a26、O48 v3 consumer 追従)。types/adapters/db/buildPublicStatus に summary を配線 (+16 tests、全 353 green)。残 = **prod 適用 = `db:push` で `services.summary` 列を本番反映 + redeploy** = Class B (release 人手ゲート)。それ以外の service-hub 自律 (no-key Class A) 作業は枯渇。
- 最終更新セッション: D20260618_002_resume_continuous
- 最終更新時刻: 2026-06-18
- 完了フェーズ: [Phase1, Phase1.5, Phase2, Phase3 実装, Phase4 デプロイ(**16 回完了**): 1st-9th + 10th (biz-charts) + 11th (fix C20260601-002 multi-series) + 12th (hotfix C20260601-003 snapshot conflict) + **13th=D20260607_004 (収益指標表示 C20260607-001)** + **14th=D20260607_006 (収益推移chart + collect unit-resilience hotfix C20260607-002)** + **15th=D20260608_005 (chart-ux 時間軸統一+期間選択)** + **16th=D20260608_009 (chart-colors 線色palette)**、Clerk production instance / sk_live_* 稼働中]
- デプロイ状況: ✅ 公開 URL = https://service-hub.givers.work (custom domain、Clerk production、live キー稼働中、**16th deploy = chart-colors 本番反映済**)。**summary-projection (8e97a26) は未デプロイ** = 次 release で `services.summary` 列 db:push + redeploy 要。
- 次の推奨コマンド: **service-hub は実質 P5 完了**。残 service-hub 作業 = summary-projection の **prod 反映** (`/flow:release` で `db:push` services.summary + redeploy、Class B 人手承認、要 release-pre 監査)。下流 = shipyard [論点-010] consumer (別 PJ、DB migration + StatusCard summary 表示)。
- Open 論点: 001✅/002✅/003✅/004✅/005✅ 解決済 + **006 (service-info summary v3 consumer 追従) = 8e97a26 で実装完了** (types+受信+公開 status API 配線、prod 反映待ち = Class B)。
- 残ゲート: P4.4 Design ✅ green / P4.45 Wording ✅ defer (internal) / P4.46 Auth-impl 不発火 (単一 owner) / **P4.7 Release = summary-projection prod 反映が残** (db:push + redeploy、要 release-pre full audit + secure) / P4.8 Promote 不発火 (非公開) / 既知 Low: queries.test.ts tsc TS2578 (deploy 非ブロッカー)。
- release-pre 必須監査 (CF-009): 直近 full = AUDIT_20260601_1229 (0/0)。以降 13th〜16th deploy + summary-projection commit (8e97a26) が乗っており **summary prod 反映前に release-pre full audit + secure が必要** (最新 AUDIT 参照 commit ≠ HEAD)。
- audit 検出常習化 (CHRONIC 注記): SCENARIO §5 drift が AUDIT_20260610_0805 で再発 (High) → **本 reconcile で解消** (12th→16th + summary-projection 反映)。
