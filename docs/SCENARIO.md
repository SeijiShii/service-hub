...
- 現在フェーズ: **Phase 4 (Release gate ✅ 21st deploy 完了)** — 21 回 deploy 完了済、本番 https://service-hub.givers.work 稼働中 (internal tool)。13th〜16th deploy (2026-06-07/08) = 収益指標 / 収益推移chart+collect hotfix / chart-ux / chart-colors。17th deploy (2026-06-18) = summary-projection [論点-011]。18th deploy (2026-06-18) = feedback-inbox [論点-007]/O67 本番反映。19th = /feedback バッジ token 化。20th deploy (2026-06-18) = revise inbox-ux 本番反映 (統合一覧明示)。21st deploy (2026-06-19) = revise inbox-pull-source 本番反映 — 無登録 feedback ソース (env HUB_FEEDBACK_SOURCES) で shipyard を services 登録なしで pull 有効化 + inbox ホームリンク + その場「今すぐ pull」。**22nd deploy (2026-06-19、dpl_b9e5egd5K5Q56NBbsMaoXhyViasA) = revise inquiries-reply-channel 本番反映** — source kind 追加で shipyard を `/api/hub/inquiries` 消費に切替 (env kind:inquiries)、email(生)+adminUrl+subject を context jsonb に取り込み inbox に返信導線 (メール返信 / shipyard admin 返信)。unit 427 + E2E 9 green、smoke green = frontend 200 / feedback 200 / 401 authed gates。
...
- 最終更新セッション: D20260618_023_release_inbox-ux
- 最終更新時刻: 2026-06-18
- 完了フェーズ: [Phase1, Phase1.5, Phase2, Phase3 実装, Phase4 デプロイ(**20 回完了**): 1st-16th + 17th (summary-projection [論点-011]) + 18th (feedback-inbox [論点-007]/O67、feedback_items db:push + dpl_7rAUwePWVhy3jdw99BfBtTRa3dqj) + 19th (/feedback バッジ token 化 CF-20260618-008) + **20th=D20260618_023 (revise inbox-ux 統合一覧 + styling、code-only)**、Clerk production instance / sk_live_* 稼働中]
- デプロイ状況: ✅ 公開 URL = https://service-hub.givers.work (custom domain、Clerk production、live キー稼働中、18th = feedback-inbox 本番反映、19th = /feedback バッジ token 化、**20th deploy (2026-06-18) = revise inbox-ux 本番反映 (統合一覧明示 = 件数サマリ + token 絞り込みバー + kind segmented chips)**、post-deploy smoke green = frontend 200 / feedback-inbox 401 authed / public-status 200)。
- 次の推奨コマンド: **service-hub は P5 完了** (feedback-inbox [論点-007] + inbox-ux revise とも本番反映済)。HUB 側に残作業なし。次の一手は別 PJ/別 repo = (1) producer 各サービスが `GET /api/hub/feedback` 実装 (O66、各サービス `/flow:revise`) → feedback が実際に流れ始める、(2) Shipyard 専用 adapter ([論点-FI-4])、(3) shipyard [論点-010] summary consumer。
- Open 論点: 001✅/002✅/003✅/004✅/005✅/006✅/**007✅ 全 7 件解決済** (007 = feedback-inbox consumer 追従、18th deploy 本番反映 + inbox-ux revise (20th) で統合一覧 UX 完成)。
- 残ゲート: P3.7 Spec-review ✅ (feedback-inbox 905) / P4.4 Design ✅ green (inbox-ux restyle 視覚レビュー green、token-conformance) / P4.45 Wording ✅ defer (internal) / P4.46 Auth-impl 不発火 (単一 owner) / P4.5 E2E ✅ (feedback-inbox 4 green) / **P4.7 Release ✅ 20th deploy 完了** (revise inbox-ux 本番反映済、smoke green) / P4.8 Promote 不発火 (非公開) / 既知 Low: queries.test.ts tsc TS2578 (deploy 非ブロッカー)。
