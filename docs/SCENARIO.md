...
- 現在フェーズ: **Phase 4 (Release gate ✅ 20th deploy 完了)** — 18 回 deploy 完了済、本番 https://service-hub.givers.work 稼働中 (internal tool)。13th〜16th deploy (2026-06-07/08) = 収益指標 / 収益推移chart+collect hotfix / chart-ux / chart-colors。17th deploy (2026-06-18) = summary-projection [論点-011]。18th deploy (2026-06-18) = feedback-inbox [論点-007]/O67 本番反映 (feedback_items 本番 Neon 追加 + /api/feedback/inbox LIVE。.vercelignore で test を関数除外し Hobby 12 関数上限を回避)。19th = /feedback バッジ token 化。**20th deploy (2026-06-18) = revise inbox-ux 本番反映** (統合一覧明示 = 件数サマリ + token 絞り込みバー + kind segmented chips、code-only redeploy、smoke green = frontend 200 / feedback-inbox 401 authed / public-status 200)。
