...
| [revise_chart-ux_20260608_axis-period-usd-cleanup/](./revise_chart-ux_20260608_axis-period-usd-cleanup/) | revise | chart-ux | **実装完了 + E2E green (2026-06-08、unit 331 / E2E 16 pass)** | 上部 chart 3 改善: (1) 縦並び複数 chart の **X 時間軸を共有 domain で統一**、(2) **期間セレクタ**(全期間/30日/7日、既定30日、`?period` で since 切替)、(3) データ未取得の **usd 系 3 chart (課金額/コスト/採算) を削除** → ユーザー数(mau)+収益(¥) の2枚に集約。DB/公開API/metric保存は不変・migration 不要・互換維持。一覧採算列は据置 ([論点-001]) | [INDEX](./revise_chart-ux_20260608_axis-period-usd-cleanup/INDEX.md) |
| [revise_chart-colors_20260608_series-palette/](./revise_chart-colors_20260608_series-palette/) | revise | chart-colors | **テスト完了 (2026-06-08、unit 337 + E2E 17/17 green)** | chart 線色 palette (`CHART_SERIES_COLORS`) を **暖寒交互順に並べ替え** (青→橙→緑→ピンク…) + near-dup (シアン#34d3a0≒緑#34d399) を#22d3ee へ解消 + 紫追加。色数8維持。先頭3色が青〜緑に固まり 2〜3 service で「青と緑だけ」に見える問題を解消。純視覚・互換維持・migration 不要・revert で rollback。tokens.test.ts で不変条件固定 | [INDEX](./revise_chart-colors_20260608_series-palette/INDEX.md) |
