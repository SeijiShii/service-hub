...
| [revise_last-deploy-col_20260530_chart-to-column/](./revise_last-deploy-col_20260530_chart-to-column/) | revise | last-deploy-col | **実装完了 + 9th deploy 済 (2026-05-30、unit 全 297 / dashboard E2E 4/4、本番反映)** + spec-review (905)。E2E で既存 fixtures drift (charts 欠落) も reconcile |
| [revise_biz-charts_20260530_revenue-cost-profit/](./revise_biz-charts_20260530_revenue-cost-profit/) | revise | biz-charts | 設計完了 (2026-05-30、tdd 待ち) | 上部 chart を**ビジネス指標化**: `up`(死活、status 列が担う)/`db_storage_bytes` を外し **ユーザー数(mau)/課金額(revenue)/コスト(ai_cost)/採算(profit 派生)** の4枚に。採算は revenue−cost の派生系列を buildCharts で合成、見出しを日本語ラベル化。MetricChart に label prop (optional、service-detail 互換) | [INDEX](./revise_biz-charts_20260530_revenue-cost-profit/INDEX.md) | `last_deploy_at` を上部 chart から除外 (4→3 枚) し、下部一覧テーブルに**「最終デプロイ」日時カラム**を additive 追加。デプロイ時刻は単一値で折れ線不向きのため timeseries-topchart の一部を差し戻し。表示層のみ・DB/公開 API 不変。spec-review: 列データソース別系統確定 (R2)・api 漏れ補完 (R3)・formatJst 再利用 (R4)・service-detail chart 維持 (R1) | [INDEX](./revise_last-deploy-col_20260530_chart-to-column/INDEX.md) |
