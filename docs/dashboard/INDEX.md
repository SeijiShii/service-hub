...
| [revise_chart-ux_20260608_axis-period-usd-cleanup/](./revise_chart-ux_20260608_axis-period-usd-cleanup/) | revise | chart-ux | **設計完了 (2026-06-08、tdd 待ち)** | 上部 chart 3 改善: (1) 縦並び複数 chart の **X 時間軸を共有 domain で統一**、(2) **期間セレクタ**(全期間/30日/7日、既定30日、`?period` で since 切替)、(3) データ未取得の **usd 系 3 chart (課金額/コスト/採算) を削除** → ユーザー数(mau)+収益(¥) の2枚に集約。DB/公開API/metric保存は不変・migration 不要・互換維持。一覧採算列は据置 ([論点-001]) | [INDEX](./revise_chart-ux_20260608_axis-period-usd-cleanup/INDEX.md) |
