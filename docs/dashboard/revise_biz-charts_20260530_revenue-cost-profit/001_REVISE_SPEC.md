...
- profitability.ts の `revenue_month_usd` / `ai_cost_month_usd` キー・cost 欠落 0 扱いと**整合**させる（採算定義の二重実装にならないよう、派生ロジックは同セマンティクスを踏襲。共通化は PLAN で検討）。
...
### [論点-001] 採算の派生ロジックを profitability.ts と共通化するか
- **影響範囲**: `buildCharts`（派生 profit 系列）と `profitability.ts`（最新値 profit）
...
- **推奨**: **(a) 純関数共通化**（採算定義の単一 SoT、P86/P19 と同方針）。spec-review/tdd で確定。
- **判断期限**: 実装時（tdd）
- **担当**: -
