...
| `src/providers/feedback.ts` | feedback pull adapter (`fetchFeedback(service, deps)` → 検証 → FeedbackItem[]) | fetch (safeFetch), types | ~90 |
| `src/features/collection/runner.ts` | per-service パスに feedback pull + upsert を統合 (既存に追記) | providers/feedback, db | ~25 |
...
| `src/features/feedback-inbox/FeedbackInboxView.tsx` | 一覧 + フィルタ UI (presentational) | react | ~120 |
...
### Phase 3: collection runner 統合
- 対象: `src/features/collection/runner.ts` への feedback pull + upsert 配線
- テスト対象: per-service で metrics pull と feedback pull が両方走る、1 サービスの feedback 失敗が他サービスをブロックしない、件数/エラーが runs に記録
- ゴール: 既存 collection テスト green 維持 + feedback 統合テスト green
...
| `src/features/collection/runner.ts` | feedback pull ステップ追加 | 中 (既存 metrics pull を壊さないこと、per-service try/catch 維持)。既存 runner.test.ts を回帰確認 |
