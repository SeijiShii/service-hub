...
## 5. ユーザー判断の結果 (2026-06-18)
**「shipyard を標準 O66 producer 化 (推奨)」を選択**。
- shipyard 側 (別 repo) で `GET /api/hub/feedback` を実装し contact メッセージを `FeedbackItem[]` (kind=inquiry/feedback) で返す + HUB の `services` に shipyard を登録 (admin)。
- → HUB の既存標準 pull がそのまま取り込む。**service-hub 側に専用 adapter は不要** → **[論点-FI-4] は「不要 (closed)」** (shipyard は標準契約に乗せる方針に確定)。
- **shipyard 側 O66 実装 + HUB 登録は別 repo / admin 作業 (ユーザー所有)**。

### 5.1 調査で surface した service-hub 側の小改修 (新規)
**admin「今すぐ pull」(`api/admin/collect.ts`) が `runFeedbackCollection` を呼んでいない** — 現状 feedback の取り込みは日次 cron (`api/cron/collect.ts`、00:00 UTC) のみ。よって shipyard が O66 + 登録を済ませても、運営者が**手動 pull で即テストできない** (日次 cron 待ち)。これは本 claim と同じ「空のまま」混乱を再発させる。
→ **service-hub 側 revise 候補**: admin force-pull に `runFeedbackCollection` を配線し、「今すぐ pull」で metrics + feedback の両方を取り込む (cron と同じ並行 invoke パターン)。Class A・小改修。
