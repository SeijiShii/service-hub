...
import { createDb, latestPerService, openAlerts, recentRuns } from "../../src/db/index.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const [latest, alerts, runs] = await Promise.all([
    latestPerService(db), openAlerts(db), recentRuns(db, 1),
  ]);
  return res.status(200).json(buildDashboard(services, latest, alerts, runs[0]));
}
